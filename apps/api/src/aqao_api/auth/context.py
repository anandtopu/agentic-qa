...
from aqao_api.auth.roles import Role

TENANT_HEADER = "X-AQAO-Tenant-Id"
USER_HEADER = "X-AQAO-User-Id"
ROLE_HEADER = "X-AQAO-Role"
CORRELATION_HEADER = "X-AQAO-Trace-Id"
...
def _parse_role(raw: str | None) -> Role | None:
    """Parse the X-AQAO-Role header into a Role enum value, or None
...
async def get_request_context(
    x_aqao_tenant_id: str | None = Header(default=None, alias=TENANT_HEADER),
    x_aqao_user_id: str | None = Header(default=None, alias=USER_HEADER),
    x_aqao_role: str | None = Header(default=None, alias=ROLE_HEADER),
    x_aqao_trace_id: str | None = Header(default=None, alias=CORRELATION_HEADER),
) -> RequestContext | None:
    """Resolve a context from headers, or return ``None`` if no tenant claim."""
    tenant_id = _parse_uuid(x_aqao_tenant_id, header_name=TENANT_HEADER)
    if tenant_id is None:
        return None
    user_id = _parse_uuid(x_aqao_user_id, header_name=USER_HEADER)
    role = _parse_role(x_aqao_role)
    return RequestContext(
        tenant_id=tenant_id,
        user_id=user_id,
        correlation_id=x_aqao_trace_id,
        role=role,
    )
...
async def require_request_context(
    ...
    resolved = context or await get_request_context(
        x_aqao_tenant_id=x_aqao_tenant_id,
        x_aqao_user_id=x_aqao_user_id,
        x_aqao_role=x_aqao_role,
        x_aqao_trace_id=x_aqao_trace_id,
    )
    if resolved is None:
        raise HTTPException(
