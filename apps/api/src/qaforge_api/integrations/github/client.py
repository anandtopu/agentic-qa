...
def get_github_client() -> GitHubClient:
    ...
    ``QAFORGE_GITHUB_APP_ID`` and ``QAFORGE_GITHUB_APP_PRIVATE_KEY_PATH``;
    the API will use the HTTP client automatically.
    """
    settings = get_settings()
    log = structlog.get_logger("qaforge_api.integrations.github")
    ...
    log.warning(
        ...
        reason="QAFORGE_GITHUB_APP_ID or private key path not configured",
    )
