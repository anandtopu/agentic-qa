"""QAForgeClient + AsyncQAForgeClient — Story 5.3.
...
class _Transport:
    ...
    def _headers(self, extra: dict[str, str] | None = None) -> dict[str, str]:
        headers = {
            ...
            "X-QAForge-Tenant-Id": self.tenant_id,
            "Content-Type": "application/json",
            "Accept": "application/json",
        }
        if self.role is not None:
            headers["X-QAForge-Role"] = self.role
        if extra:
...
@dataclass(slots=True)
class QAForgeClient:
    """Synchronous QAForge SDK client.
...
@dataclass(slots=True)
class AsyncQAForgeClient(QAForgeClient):
    """Async variant — same surface, awaitable methods.

    Phase-5 ships the same shape as :class:`QAForgeClient` so callers
...
def _default_sender() -> Callable[..., dict[str, Any]]:
    ...
    def _send(
        *,
        ...
        import httpx
        from qaforge_sdk.errors import error_for_status
        ...
        if response.status_code >= 400:
            ...
            raise error_for_status(
                ...
                trace_id=response.headers.get("X-QAForge-Trace-Id"),
...
__all__ = ["AsyncQAForgeClient", "QAForgeClient"]
