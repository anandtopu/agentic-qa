"""QAForge execution-plane tools (Phase 1).

* :mod:`qaforge_tools.credentials` — credential broker that bridges the
  Control Plane's per-environment variables (Story 1.1.3) into tool runs
  while registering each value with the redactor.
* :mod:`qaforge_tools.newman` — Postman/Newman runner.
* :mod:`qaforge_tools.pytest_runner` — pytest+httpx runner.

Each module is structured so production callers depend on a Protocol
and tests inject a stub.
"""

from qaforge_tools.credentials import (
    CredentialBroker,
