...
Includes a hidden HTML-comment marker (``AQAO_PR_COMMENT_MARKER``)
so the GitHub Action can find an existing Agentic QA Orchestrator comment and update
...
from aqao_agents.reporter.types import GoNoGo, RunReportContext

PR_COMMENT_MARKER = "<!-- aqao:pr-comment:v1 -->"
"""HTML comment GitHub Actions look for to identify the Agentic QA Orchestrator comment."""
...
class PrCommentRenderer:
    ...
    def render(
        ...
        *,
        ...
        lines.append("## Agentic QA Orchestrator — Run Summary")
        lines.append("")
        lines.append(
        ...
        lines.append(
            f"<sub>Agentic QA Orchestrator run `{context.test_run_id}` · "
