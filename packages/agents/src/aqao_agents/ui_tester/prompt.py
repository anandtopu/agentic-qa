...
from aqao_agents.ui_tester.schema import GeneratedUiTestSpec

PROMPT_VERSION = "1.0.0"

SYSTEM = """\
You are Agentic QA Orchestrator's UI Testing Agent. You convert UI test cases into a
...
2. ``source_code`` must be valid TypeScript for ``@playwright/test``. It must:
   ...
   - read the base URL from ``process.env.AQAO_UI_BASE_URL``;
   - read auth from ``process.env.AQAO_UI_AUTH_TOKEN`` when needed and
