...
* No ``AQAO_UI_BASE_URL`` env reference (an explicit instruction
...
_BASE_URL_REF = re.compile(r"process\.env\.AQAO_UI_BASE_URL")
...
def validate_spec(source: str) -> None:
    ...
    if not _BASE_URL_REF.search(stripped.text):
        raise GeneratedSpecInvalid("spec must read process.env.AQAO_UI_BASE_URL — none found")
