...
2. Check the action logs for `start test_run failed: 401` — your
   `QAFORGE_TOKEN` secret is missing or expired.
...
```bash
curl https://api.qaforge.ai/api/v1/test-runs/$RUN_ID/report
```
...
```bash
curl 'https://api.qaforge.ai/api/v1/audit?resource_type=test_run&resource_id=$RUN_ID' \
  -H "Authorization: Bearer $QAFORGE_TOKEN" \
  -H "X-QAForge-Tenant-Id: $QAFORGE_TENANT_ID"
```
...
```bash
curl 'https://api.qaforge.ai/api/v1/usage/summary?workspace_id=$WS&since=2026-04-01' \
  -H "Authorization: Bearer $QAFORGE_TOKEN" \
  -H "X-QAForge-Tenant-Id: $QAFORGE_TENANT_ID"
```
...
settings doesn't match the one in QAForge. Re-issue via:

```bash
curl -X POST \
  https://api.qaforge.ai/api/v1/repositories/$REPO_ID/webhook-secret \
  -H "Authorization: Bearer $QAFORGE_TOKEN" \
  -H "X-QAForge-Tenant-Id: $QAFORGE_TENANT_ID"
```
...
* Open an issue at <https://github.com/qaforge/qaforge/issues> with
