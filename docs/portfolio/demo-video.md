...
`checkout` discount logic. We watch Agentic QA Orchestrator classify the failure as
...
| 2 | Action triggers (GitHub Actions tab) | 3s | `aqao-action@v1` running. |
| 3 | API live tail (terminal) | 6s | `aqao_api` JSON logs: ingest → plan → run → classify → report. |
...
| 9 | PR comment lands | 6s | Single Agentic QA Orchestrator comment with risk band, top drivers, evidence link. |
| 10 | Audit log + cost | 5s | `usage_summary`: $0.42 / $5.00; audit log with HMAC signature ✓ green. |

## Voice-over key lines

> "Agentic QA Orchestrator reads the PR, plans the right tests, runs them, and tells
